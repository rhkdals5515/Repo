...
public class D05_Comparator {
	...
	public static void main(String[] args) {
		...
		Collections.sort(grapes, new podoDescending());
		System.out.println("\n #### 정렬 후####");
...
class podoDescending implements Comparator<Grape>{
...
class 포도분류기_GNO기준_내림차순 implements Comparator<Grape> {
	...
	public int compare(Grape o1, Grape o2) {
		...
		// 문자열의 알파벳 순서 비교는 이미 String클래스의 compareTo메서드에 구현되어 있다.
...
class Grape {
	...
	/*
	 	# toString()
	 	 ...
	 	 - 이 인스턴스를 문자열로 포현한다면? 을 구현해놓는 메서드
	 	 - System.out.println()은 사실 전달한 인스턴스의 toString()
	 	      메서드의 결과를 콘솔에 출력하는 메서드이다
	 	 - Object 클래스의 toString() 동작은 
	 	 "클래스 명@주소값"이다
	 */
	
	@Override
	public String toString() {
		return this.gno + "/" + this.fruit + "/" + this.locationNames[location] + "(" + location + ")";
	}
}
